...
/**
 * Gets all route names from the navigation stack, including nested navigators.
 * This function recursively explores the stack and returns all the route names.
 */
export function getAllRouteNames(
  ...
  // Iterate through all the routes in the stack
  state.routes.forEach(route => {
    routeNames.push(route.name)

    // If the route has a nested state, recursively fetch route names from the nested navigator
    if (route.state) {
      routeNames = routeNames.concat(getAllRouteNames(route.state as NavigationState<AppStackParamList>))
    }
  })
...
/**
 * Function to reset all active routes all at once
 */
export const resetAllActiveRoutes = ()=> {
  const routes = getAllRouteNames(navigationRef.getState())

  const modifiedRoutes = routes.map((route) => {
    return { name: route }
  })

  resetRoot({ index: 0, routes: modifiedRoutes })
}
